...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      // GATILHO AUTOMÁTICO CRÍTICO: Dispara exatamente 5 segundos antes da vela fechar
      // Se secondsToNextCandle for 5, significa que faltam 5 segundos para a próxima vela.
      if (!isGenerating && tfSeconds === 5) {
        const candleId = `${selectedPair}-${selectedTimeframe}-${now.getHours()}-${now.getMinutes()}-${now.getSeconds()}`;
        if (lastTriggeredCandleRef.current !== candleId) {
          lastTriggeredCandleRef.current = candleId;
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] flex flex-col">
            ...
            <div className="hidden md:flex flex-col border-l border-white/10 pl-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">
                  ALGORITMO ATIVO: MONITORANDO MERCADO
                </span>
              </div>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
        <aside className="lg:col-span-1 space-y-6">
          <div className="glass p-6 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
            <h2 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6 border-b border-white/5 pb-2">Parâmetros de Análise</h2>
            
            <div className="space-y-5">
              <div>
                <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-widest">Ativo Financeiro</label>
                <select
                  value={selectedPair}
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-widest">Tempo de Vela (TF)</label>
                  ...
                  {TIMEFRAMES.map(tf => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setActiveSignal(null); // Reseta ao trocar timeframe para nova análise
                      }}
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
                <p className="text-[10px] text-blue-400 font-black uppercase tracking-widest text-center">
                  Scanner Inteligente Ligado
                </p>
              </div>
            </div>
          </div>

          <div className="glass p-6 rounded-3xl border border-white/5 bg-gradient-to-b from-transparent to-green-500/5">
            <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Integridade do Sinal</h2>
            <div className="space-y-4">
               <div className="flex justify-between text-[10px] font-bold">
                 <span className="text-slate-500">CONEXÃO IA</span>
               ...
               <div className="w-full bg-slate-900 rounded-full h-3 overflow-hidden border border-white/5">
                  <div 
                    className="h-full logo-gradient-bg transition-all duration-1000 shadow-[0_0_10px_rgba(74,222,128,0.5)]"
                    style={{ width: '100%' }}
                  ></div>
               </div>
               <p className="text-[10px] text-slate-400 leading-relaxed font-medium opacity-80 uppercase tracking-tighter">
                O sinal será disparado automaticamente quando o cronômetro atingir 00:05.
               </p>
            </div>
          </div>
        </aside>
        ...
        <div className="lg:col-span-3">
          <div className="flex justify-between items-end mb-8">
            <div className="flex flex-col gap-1">
               <h2 className="text-2xl font-black text-white tracking-tighter uppercase">Painel de Sinais Elite</h2>
               <div className="flex gap-3 items-center">
                  <span className="flex items-center gap-1.5 bg-green-500/10 text-green-400 text-[10px] font-black px-2 py-1 rounded-md border border-green-500/20 uppercase tracking-widest">
          ...
          {!activeSignal ? (
            <div className="glass rounded-[40px] h-[550px] flex flex-col items-center justify-center text-center p-12 border-dashed border-2 border-white/5 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-2xl font-black text-white mb-4 tracking-tighter uppercase">
                  {isGenerating ? 'IA Processando Entrada' : 'Aguardando Janela de Oportunidade'}
                </h3>
                <p className="text-slate-500 text-base max-w-sm mx-auto leading-relaxed font-medium">
                  {isGenerating 
                    ? 'O algoritmo está validando a direção da próxima vela com base no fluxo institucional.' 
                    : 'Aguarde os últimos 5 segundos da vela atual para receber a confirmação de entrada.'}
                </p>
              </div>
            </div>
          ) : (
            ...
            >
              ...
              <div className="mt-12 p-8 glass rounded-[32px] border border-green-500/20 bg-green-500/5 shadow-inner">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h4 className="text-sm font-black text-white uppercase tracking-[0.2em]">Execução de Alta Performance</h4>
                </div>
                <ul className="space-y-4">
                  <li className="flex gap-4 text-xs text-slate-400 items-center">
                    <span className="h-6 w-6 rounded-full bg-slate-800 flex items-center justify-center text-green-400 font-black flex-shrink-0 border border-white/5">1</span>
                    Selecione o ativo <span className="text-white font-bold">{activeSignal.pair}</span> na sua plataforma.
                  </li>
                  <li className="flex gap-4 text-xs text-slate-400 items-center">
                    <span className="h-6 w-6 rounded-full bg-slate-800 flex items-center justify-center text-green-400 font-black flex-shrink-0 border border-white/5">2</span>
                    Ajuste o tempo para <span className="text-white font-bold">{activeSignal.timeframe}</span>.
                  </li>
                  <li className="flex gap-4 text-xs text-slate-400 items-center">
                    <span className="h-6 w-6 rounded-full bg-slate-800 flex items-center justify-center text-green-400 font-black flex-shrink-0 border border-white/5">3</span>
                    Clique no botão de {activeSignal.direction === 'CALL' ? 'COMPRA' : 'VENDA'} assim que o relógio marcar <span className="text-white font-black">{activeSignal.entryTime}</span>.
                  </li>
                </ul>
              </div>
            </div>
      ...
      <footer className="glass border-t border-white/5 py-4 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] text-slate-500 uppercase font-black tracking-[0.3em]">
          <div className="flex items-center gap-8">
            <span className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-green-500"></span>
              Ultra Trade Signal VIP: Monitoramento 24/7 Ativo
            </span>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-white opacity-80 uppercase">Terminal de Sinais Profissional</span>
          </div>
        </div>
      </footer>
    </div>
